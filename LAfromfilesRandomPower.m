...
function [] = LAfromfilesRandomPower(inits,fileID,P1max,P2max ,num_iter,rounds,sig1,sig2,sige,sigl1,sigl2,thresh1,a,b)
...
for ui=1:num_iter 
    filex = fopen(strcat('.\Case1_RicFactor8.4\channel',' ',num2str(ui-1),' ','.txt'),'r');
    num = fscanf(filex,'%e');
    h_ab = num(1) + (1j*num(2));
    h_ae = num(3) + (1j*num(4));
    h_be = num(5) + (1j*num(6));
...
for i=a:b
    ...
    for ui=1:num_iter
        ...
        [s,optP1,optP2] = RadomPower(inits,rounds,L,P1max,P2max,sig1,sig2,sige,sigl1,sigl2,H,G,E1,E2,thresh1);
        sum = sum+s;
